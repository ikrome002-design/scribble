...
function generateOtp($user, $minutes = 30, $send = true)
{
    $code = rand(100000, 999999);
    $expiry = Carbon::now()->addMinutes($minutes)->format('Y-m-d H:i:s');
    ...
    $subject = 'OTP';
    $message = "Please use this OTP. The OTP expires after $minutes minutes.";
    if ($send) {
        Mail::to($user->email)->send(new General($subject, $name, $message, null, null, $code));
    }
    return $code;
}
...
function proSmsNotSent()
{
    return ProSmsNotSent::whereHas('proSubscription', function ($q) {
