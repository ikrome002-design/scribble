...
use App\Mail\GovernmentSenderId;
...
class SMSController extends Controller
{
    ...
    //======================================================================
    // send email to government for approval eFunction Start Here
    //======================================================================
    public function sendSenderIdGovernment(Request $request, $sender_id)
    {
        $this->validate(
            $request,
            [
        ...
        $files = SenderIdFiles::where('sender_id', $sender_id)->get();
        $files_email = [];
        if ($files) {
        ...
        try {
            \Mail::to(app_config('Email'))->send(new GovernmentSenderId($request->email, $request->subject, $request->message, $files_email));

            return back()->with('message', 'Request send successfully to the government/Company');
        } catch (\Exception $ex) {
            return back()->withErrors('Message was not sent. Please try again.');
        }
    }
    //======================================================================
    ...
    public function editPlan($id)
    {
        ...
        $client_groups = ClientGroups::all();
        $plans = Plan::all();

        return view('admin.edit-sms-price-plan', compact('price_plan', 'client_groups', 'plans'));
    }
    ...
    public function updatePlan(Request $request, $id)
    {
        ...
        $v = \Validator::make($request->all(), [
            'unit_from' => 'required|numeric',
            'unit_to' => 'required|numeric',
        ...
        if ($v->fails()) {
            ...
            return redirect('sms/edit-sms-price-plan/' . $id)->withErrors($v->errors());
        }

        $price_plan =  SMSPricePlan::find($id);

        if ($price_plan) {
            $price_plan->unit_from = $request->unit_from;
            $price_plan->unit_to =  $request->unit_to;
            $price_plan->transaction_fee = $request->transaction_fee;
            $price_plan->discount_type    = $request->disc_amount_charge;
            $price_plan->apply_discount = $request->discounts;
            $price_plan->discount_amount = $request->discount_amount;
            $price_plan->govt_charges_type    = $request->Government_Charges_Type;
            $price_plan->apply_govt_charges = $request->apply_charges;
            $price_plan->govt_charges_amt = $request->govt_charges_amount;
            $price_plan->client_id = $request->client;
            $price_plan->client_group_id = $request->client_group;
            $price_plan->plan_id = $request->plan;
            $price_plan->show_client = $request->show_in_client;
            $price_plan->price = $request->price;
            $price_plan->status = $request->status;
            $price_plan->save();
            $message = "SMS price plan has been updated successfully";
        } else {
    ...
    public function deleteSmsPlan($id)
    {
        ...
        return redirect('sms/price-plan')->with([
            'message' => 'SMS price plan has been deleted successfully'
        ]);
    }

    public function viewPlan($id)
    {
        $price_plan =  SMSPricePlan::where('sys_sms_price_plan.id', $id)->join('sys_clients', 'sys_clients.id', '=', 'sys_sms_price_plan.client_id')
            ->join('sys_client_groups', 'sys_client_groups.id', '=', 'sys_sms_price_plan.client_group_id')->join('sys_plan_name', 'sys_plan_name.id', '=', 'sys_sms_price_plan.plan_id')->select('sys_sms_price_plan.*', 'sys_clients.fname', 'sys_clients.lname', 'sys_client_groups.group_name', 'sys_plan_name.plan_name')->first();

        return view('admin.view-sms-price-plan', compact('price_plan'));
    }
    ...
    public function addPricePlan()
    {
        ...
        $client_group = ClientGroups::select('id', 'group_name')->where('status', '=', 'Yes')->get();
        $plans = Plan::all();

        return view('admin.add-price-plan', compact('client_group', 'plans'));
    }

    //=====================================================================
    // addpriceName function start here
    //====================================================================

    public function addPlanName()
    {
        return view('admin.add-plan-name');
    }

    //=====================================================================
    // addpriceName function start here
    //====================================================================
    public function postNewPlanName(Request $request)
    {
        $self = 'add-plan-name';
        if (Auth::user()->username !== 'admin') {
            $get_perm = Permission::permitted($self);
        ...
        if ($v->fails()) {
            return redirect('sms/add-plan-name')->withErrors($v->errors());
        }
    ...
    {
        $plan = PlanName::get();
        return view('admin.plan', ['plan' => $plan]);
    }
    ...
    public function deletePlan($id)
    {
        ...
        //checking plan is using in sms price or not
        $sms_price = SMSPricePlan::where('plan_id', $id)->first();

        if (isset($sms_price) && !empty($sms_price)) {
            return redirect('sms/plan')->with([
                'message' => language_data('Plan cannot be deleted it is used in sms price plan')
            ]);
        } else {
            $fetech_plan = PlanName::where('id', $id)->first();

            if (isset($fetech_plan) && !empty($fetech_plan)) {
                $fetech_plan->delete();
                return redirect('sms/plan')->with([
                    'message' => language_data('Plan has been deleted')
                ]);
            } else {
                return redirect('sms/plan')->with([
    ...
    public function airtimeBundle()
    {
        ...
        return view('admin.sms-airtime-bundle', compact('airtime_bundle'));
    }
    ...
    public function editAirtime($id)
    {
        ...
        $airtime_bundle = AirtimeBundle::find($id);
        $client_groups = ClientGroups::all();
        $plans = Plan::all();
        return view('admin.edit-airtime-bundle', compact('airtime_bundle',  'client_groups', 'plans'));
    }
    ...
    public function updateAirtime(Request $request, $id)
    {
        ...
        if ($v->fails()) {
            return redirect('sms/edit-airtime-bundle/' . $id)->withErrors($v->errors());
        }

        $airtime = AirtimeBundle::where('id', $id)->first();

        if (isset($airtime) && !empty($airtime)) {
            $airtime->units = $request->units;
            $airtime->transaction_fee = $request->transaction_fee;
            $airtime->discount_type =  $request->disc_amount_charge;
            $airtime->apply_discount = $request->discounts;
            $airtime->discount_amount = $request->discount_amount;
            $airtime->govt_charges_type = $request->Government_Charges_Type;
            $airtime->apply_govt_charges = $request->apply_govt_charges;
            $airtime->govt_charges_amt = $request->govt_charges_amount;
            $airtime->client_id = $request->client;
            $airtime->client_group_id = $request->client_group;
            $airtime->plan_id = $request->plan;
            $airtime->price = $request->price;
            $airtime->show_client = $request->show_in_client;
            $airtime->status = $request->status;
            $airtime->bundle_name = $request->bundle_name;
            $airtime->save();
            $message = "Airtime Bundle has been updated successfully";
        } else {
            $message = "Record not found";
        }

        return redirect('sms/airtime-bundle')->with([
            'message' => $message
        ]);
    }
    ...
    public function view($id)
    {
        ...
        $airtime_bundle =  AirtimeBundle::where('sys_airtime_bundle.id', $id)
            ->join('sys_clients', 'sys_clients.id', '=', 'sys_airtime_bundle.client_id')
            ->join('sys_client_groups', 'sys_client_groups.id', '=', 'sys_airtime_bundle.client_group_id')
            ->join('sys_plan_name', 'sys_plan_name.id', '=', 'sys_airtime_bundle.plan_id')
            ->select('sys_airtime_bundle.*', 'sys_clients.fname', 'sys_clients.lname', 'sys_client_groups.group_name', 'sys_plan_name.plan_name')
            ->first();

        return view('admin.view-airtime-bundle', compact('airtime_bundle'));
    }

    public function deleteAirtime($id)
    {
        $bundle = AirtimeBundle::find($id);
        $bundle->delete();

        return redirect('sms/airtime-bundle')->with([
            'message' => 'Airtime Bundle has been removed successfully'
        ]);
    }

    //======================================================================
    // addAirtimeBundle Function Start Here
    //======================================================================

    public function addAirtimeBundle()
    {
        $client_groups = ClientGroups::all();
        $plans = Plan::all();

        return view('admin.airtimebundle',  compact('client_groups', 'plans'));
    }

    //======================================================================
    // postAirtime Function Start Here
    //======================================================================

    public function postAirtime(Request $request)
    {
        $self = 'add-airtime-bundle';
        if (Auth::user()->username !== 'admin') {
            $get_perm = Permission::permitted($self);
        ...
        if ($v->fails()) {
            return redirect('sms/add-airtime-bundle')->withErrors($v->errors());
        }



        $bundle = new AirtimeBundle;
        $bundle->units = $request->units;
        $bundle->price = $request->price;
        $bundle->transaction_fee = $request->transaction_fee;
        $bundle->discount_type = $request->disc_amount_charge;
        $bundle->apply_discount = $request->discounts;
        $bundle->discount_amount = $request->discount_amount;
        $bundle->govt_charges_type = $request->Government_Charges_Type;
        $bundle->apply_govt_charges = $request->apply_charges;
        $bundle->govt_charges_amt = $request->govt_charges_amount;
        $bundle->client_id = $request->client;
        $bundle->client_group_id = $request->client_group;
        $bundle->plan_id = $request->plan;
        $bundle->show_client = $request->show_in_client;
        $bundle->status = $request->status;
        $bundle->bundle_name = $request->bundle_name;
        $bundle->save();

        return redirect('sms/add-airtime-bundle')->with([
            'message' => language_data('Airtime bundle has been added successfully')
        ]);
    }
    ...
    public function postNewPricePlan(Request $request)
    {
        ...
        $plan->unit_from = $request->unit_from;
        $plan->unit_to = $request->unit_to;
        $plan->transaction_fee = $request->transaction_fee;
        $plan->discount_type = $request->disc_amount_charge;
        $plan->client_id = $request->client;
        $plan->client_group_id = $request->client_group;
        $plan->apply_discount = $request->discounts;
        $plan->discount_amount = $request->discount_amount;
        $plan->govt_charges_type = $request->Government_Charges_Type;
        $plan->apply_govt_charges = $request->apply_govt_charges;
        $plan->govt_charges_amt = $request->govt_charges_amount;
        ...
        return redirect('sms/price-plan')->with([
            'message' => language_data('SMS Price added successfully')
        ]);
    }
    ...
    public function deletePricePlan($id)
    {
        ...
        if ($price_plan) {
            SMSPlanFeature::where('id', $id)->delete();
            $price_plan->delete();
            return redirect('sms/price-plan')->with([
    ...
    public function postNewSMPPGateway(Request $request)
    {
        ...
        $gateway->status   = $request->status;
        $gateway->type    = 'smpp';
        $gateway->custom_smpp     = $request->custom_smpp;
        $gateway->two_way  = 'No';
        $gateway->voice    = 'No';
        $gateway->mms      = 'No';
        $gateway->save();
