...
class ClientController extends Controller
{
    ...
    public function addClient()
    {
        ...
        return view('admin.add-client', compact('clientGroups', 'sms_gateways', 'plans'));
    }
    ...
    public function addClientPost(Request $request)
    {
        ...
        $v = \Validator::make($request->all(), [
            'first_name' => 'required',  'email' => 'required', 'password' => 'required', 'cpassword' => 'required', 'phone' => 'required', 'country' => 'required', 'api_access' => 'required', 'sms_gateway' => 'required', 'sms_limit' => 'required|numeric|max:1000000000', 'reseller_panel' => 'required', 'image' => 'image|mimes:jpeg,jpg,png,gif', 'plan' => 'required|numeric'
        ]);
        ...
        if ($request->image) {
            $image_name = basename(Storage::disk('private')->putFile('/profile/client', $request->image));
        }
        ...
        if ($plan->price > 0 and $request->create_invoice == 'Yes') {
        ...
        if (is_int($client_id)) {
            ...
            if ($email_notify == 'Yes' && $email != '') {
                ...
                try {
                    \Mail::to($email)->send(new UserRegistration($name, $email, $cpassword));
    ...
    public function viewClient($id)
    {
        ...
        $plans = Plan::all();
        $buy_plan = BuyPlan::select('plan_id')->where(['cl_id' => $id])->first();
        ...
        if ($client) {
            ...
            return view('admin.client-manage', compact('client', 'invoices', 'tickets', 'sms_gateways', 'sms_transaction', 'clientGroups', 'selected_gateways', 'buy_plan', 'plans'));
        } else {
            return redirect('clients/all')->with([
    ...
    public function updateImage(Request $request)
    {
        ...
        if ($appStage == 'Demo') {
            return redirect('clients/view/' . $cmd)->with(
                'message',
                'This Option is Disable In Demo Mode'
            );
        }
        ...
        if (Auth::user()->username !== 'admin') {
            ...
            if ($get_perm == 'access denied') {
                return redirect('permission-error')->withErrors([
                    'message' => language_data('You do not have permission to view this page'),
                    'message_important' => true
                ]);
            }
        ...
        $image_name = basename($disk->putFile('/profile/client', $request->client_image));
        $client->image = $image_name;
        $client->save();
    ...
    public function updateClient(Request $request)
    {
        ...
        $v = \Validator::make($request->all(), [
            'first_name' => 'required',  'phone' => 'required', 'country' => 'required', 'api_access' => 'required', 'client_group' => 'required', 'status' => 'required', 'reseller_panel' => 'required', 'email' => 'required', 'sms_gateway' => 'required'
        ]);
        ...
        if ($client) {
            ...
            if ($request->plan_status == 'Active') {
                if ($request->plan_recurring_date < $today) {
                    return back()->withErrors('If you make a plan active, the next recurring date  must greater than today.');
                }
            }
            ...
            $client->reseller    = $request->reseller_panel;
            $client->sms_gateway = $sms_gateways_id;
            $client->plan_status = $request->plan_status;
            $client->plan_id  = $request->plan_id;
            $client->plan_recurring_date = $request->plan_recurring_date;
            $client->save();
            if ($request->create_invoice == 'Yes') {
    ...
    public function getAllClients()
    {
        ...
        return Datatables::of($clients)
            ...
            ->addColumn('datecreated', function ($cl) {
                return date('Y-M-d', strtotime($cl->created_at));
            })
