...
class AuthController extends Controller
{
    ...
    public function register()
    {
        ...
        return view('register_steps.steps', compact('plans', 'accounts'));
    }
    ...
    public function storeSteps(Request $request)
    {
        if ($request->step == 1) {
            ...
            Session::put('step_one_info', $step_one_info);
        } else if ($request->step == 2) {
            $step_two_info = [
                "package" => $request->package
            ];

            Session::put('step_two_info', $step_two_info);
            ...
            $step_three_info = [
                ...
                'email' => $request->email,
                'pass' => $request->pwd,
                'conpass' => $request->cpwd
            ];

            Session::put('step_three_info', $step_three_info);
            ...
            if (isset($request->type_of_business)) {
                $business_type = $request->type_of_business;
            } else {
                $business_type = null;
            }
            ...
            if (isset($request->type_of_school)) {
                $school_type = $request->type_of_school;
            } else {
                $school_type = $request->type_of_school;
            }
            ...
            if (isset($request->type_of_organization)) {
                $organization_type = $request->type_of_organization;
            } else {
                $organization_type = null;
            }

            $step_fourth_info = [
                'phone_number' => $request->phone_number ?: ($request->phone_number_business ?: ($request->phone_number_school ?: $request->phone_number_org)),
                'recovery_email_address' => $request->recovery_email_address ?: ($request->recovery_email_address_business ?: ($request->recovery_email_address_school ?: $request->recovery_email_address_org)),
                'date' => $request->date ?: ($request->date_business ?: ($request->date_school ?: $request->date_org ?: date('Y-m-d'))),
                'gender' => $request->gender ?: ($request->gender_business ?: ($request->gender_school ?: $request->gender_org)),
            ...
            Session::put('step_fourth_info', $step_fourth_info);
        } else {

            $step_1 =  Session::get("step_one_info");
            $step_2 =  Session::get("step_two_info");
            $step_3 =  Session::get("step_three_info");
            $step_4 = session::get("step_fourth_info");


            $errors = [];

            if (Client::where('email', '=', $step_3['email'])->exists()) {
                $errors[] = 'The email is already exist';
            }
            if ($step_3['pass'] != $step_3['conpass']) {
            ...
            $sms_gateways = SMSGateways::first()->id;
            $sms_gateways_id = json_encode([$sms_gateways], true);
            $api_key_generate    = $request->user_name . ':' . $step_3['conpass'];
            $plan = Plan::find((int)$step_2['package']);
            $client = new Client;
            $client->fname = $step_3['fname'];
            $client->lname = $step_3['lname'];
            $client->email = $step_3['email'];
            $client->account_type = $step_1['account'];

            $client->password = Hash::make($step_3['pass']);
            $client->phone = $step_4['phone_number'];
            $client->recovery_email_address = $step_4['recovery_email_address'];
            $client->date =  $step_4['date'];
            $client->gender = $step_4['gender'];
            $client->business_name = $step_4['business_name'];
            $client->business_type =  $step_4['type_of_business'];
            $client->school_name = $step_4['school_name'];
            $client->school_type =  $step_4['school_type'];
            $client->organization_name = $step_4['organization_name'];
            $client->organization_type = $step_4['organization_type'];
            ...
            if ($plan->price == 0) {
                ...
                $client->plan_id = $plan->id;
                $client->plan_status = 'Inactive';
            }
            $client->save();
            $name = $request->fname . ' ' . $request->lname;

            \Mail::to($client->email)->send(new UserRegistration($name, $client->email, $step_3['pass'], '/login'));

            //send invoice if plan is greater than 0/recurring
            if ($plan->price > 0) {
                ...
                Session::forget('step_one_info');
                Session::forget('step_two_info');
                Session::forget('step_three_info');
                Session::forget('step_fourth_info');

                return redirect('/register/invoice/' . $invoice_no)->with('message', 'Thank you for registering. Please you can pay for the package by following instruction');
            }
            Session::forget('step_one_info');
            Session::forget('step_two_info');
            Session::forget('step_three_info');
            Session::forget('step_fourth_info');
            return redirect('/login')->with('message', 'Please login to start the session');
        }
    }

    public function registerInvoice($id)
    {
        $invoice = Invoices::where('invoice_no', $id)->get()->first();
        $inv = null;
        $client = null;
        $message = null;
        $items = null;
        if (!$invoice) {
            ...
            $message = "The invoice has be been paid.";
        } else {
            $inv = $invoice;
            $client = Client::find($inv->cl_id);
            $items = Invoices::where('invoice_no', $id)->get();
        }



        return view('register_steps.invoice', compact('inv', 'client', 'message', 'items'));
    }


    //======================================================================
    // homepage Function Start Here
    //======================================================================
    ...
    public function clientGetLogin(Request $request)
    {
        ...
        // previous develoepr code
        // Auth::guard('client')->attempt($check_input, $remember)
...
';
                        ...
                        $rows       = explode("\n", $env);
                        $unwanted   = "MAIL_MAILER|MAIL_HOST|MAIL_PORT|MAIL_USERNAME|MAIL_PASSWORD|MAIL_ENCRYPTION|APP_NAME";
