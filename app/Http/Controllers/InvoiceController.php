...
class InvoiceController extends Controller
{
    ...
    public function allInvoices(InvoicesDataTable $dataTable)
    {
        ...
        return $dataTable->render('admin.all-invoices');
    }
    ...
    public function viewInvoice($id)
    {
        ...
        if ($inv) {
            ...
            return view('admin.view-invoice', compact('client', 'inv', 'items'));
        } else {
    ...
    public function editInvoice($id)
    {
        ...
        if ($inv) {
            $items = InvoiceItems::where('invoice_no', $inv->invoice_no)->get();;
            $client = Client::find($inv->cl_id);
            return view('admin.edit-invoice', compact('client', 'inv', 'items'));
        } else {
            return redirect('invoices/all')->with([
                'message' => language_data('Invoice not found'),
                'message_important' => true
    ...
    public function postEditInvoice(Request $request)
    {
        ...
        if ($inv) {
            $inv->duedate      = $request->due_date;
            $inv->tax      = $request->tax;
            $inv->discount       = $request->discount;
            $inv->subtotal     = $request->subtotal;
            $inv->total       = $request->total;
            $inv->trans_amount       = $request->trans_amount;
            $inv->status       = $request->status;
            $inv->sms_limit       = $request->sms_limit;
            $inv->save();

            $items = [];
            foreach ($request->item_id  as $key => $p) {
                $items[] = [
    ...
    public function deleteInvoice($id)
    {
        ...
        $inv = Invoices::find($id);
        if ($inv) {
            InvoiceItems::where('invoice_no', $id)->delete();
            $inv->delete();
