...
class ClientDashboardController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        $inv_unpaid         = Invoices::where('status', 'Unpaid')->where('cl_id', auth('team')->user()->cl_id)->count();
        $inv_paid           = Invoices::where('status', 'Paid')->where('cl_id', auth('team')->user()->cl_id)->count();
        $inv_cancelled      = Invoices::where('status', 'Cancelled')->where('cl_id', auth('team')->user()->cl_id)->count();
        $inv_partially_paid = Invoices::where('status', 'Partially Paid')->where('cl_id', auth('team')->user()->cl_id)->count();
        ...
        $st_pending  = SupportTickets::where('status', 'Pending')->where('cl_id', auth('team')->user()->cl_id)->count();
        $st_answered = SupportTickets::where('status', 'Answered')->where('cl_id', auth('team')->user()->cl_id)->count();
        $st_replied  = SupportTickets::where('status', 'Customer Reply')->where('cl_id', auth('team')->user()->cl_id)->count();
        $st_closed   = SupportTickets::where('status', 'Closed')->where('cl_id', auth('team')->user()->cl_id)->count();
        ...
        $sms_count   = SMSHistory::where('userid', auth('team')->user()->cl_id)->count();
        $sms_success = SMSHistory::where('userid', auth('team')->user()->cl_id)->where('status', 'like', '%Success%')->count();

        $get_campaign = Campaigns::where('user_id', auth('team')->user()->cl_id)->where('status', '!=', 'Delivered')->select('campaign_id')->get()->toArray();
        ...
        $get_reseller = Client::where('parent', auth('team')->user()->cl_id)->select('id')->get()->toArray();
        $reseller_ids = array_column($get_reseller, 'id');
        $reseller_ids[] = auth('team')->user()->cl_id;
        ...
        $recent_five_invoices = Invoices::orderBy('id', 'desc')->where('cl_id', auth('team')->user()->cl_id)->take(5)->get();
        $recent_five_tickets  = SupportTickets::orderBy('id', 'desc')->where('cl_id', auth('team')->user()->cl_id)->take(5)->get();
    ...
    {
        auth('team')->logout();
        return redirect('//staff.' . env('APP_DOMAIN'))->with([
    ...
    public function updateAvatar(Request $request)
    {
        ...
        $client = auth('team')->user();
        $image_name = basename(Storage::disk('private')->putFile('/staff', $request->image));
    ...
    public function changeLanguage($id)
    {
        ...
        if ($lang) {
            Client::where('id', auth('team')->user()->cl_id)->update(['lan_id' => $id]);
            return redirect('dashboard')->with([
