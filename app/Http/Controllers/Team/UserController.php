...
class UserController extends Controller
{
    ...
    public function addUser()
    {
        ...
        if (auth('team')->user()->client->reseller == 'No') {
        ...
        $clientGroups = ClientGroups::where('status', 'Yes')->where('created_by', auth('team')->user()->cl_id)->get();
    ...
    public function addUserPost(Request $request)
    {
        ...
        $v = \Validator::make($request->all(), [
            'first_name' => 'required', 'user_name' => 'required', 'password' => 'required', 'cpassword' => 'required', 'phone' => 'required', 'country' => 'required', 'client_group' => 'required', 'sms_limit' => 'required|numeric|min:0|max:1000000000', 'email' => 'required', 'sms_gateway' => 'required', 'image' => 'image|mimes:jpeg,jpg,png,gif'
        ]);
        ...
        if (isset($sms_gateways) && is_array($sms_gateways) && count(array_filter($sms_gateways)) <= 0) {
            return redirect('user/add')->with([
                'message' => 'At least select one sms gateway',
                'message_important' => true
            ]);
        }
        ...
        $parent = Client::find(auth('team')->user()->cl_id);
        ...
        if ($request->sms_limit != '') {
            ...
            if ($request->sms_limit < 0) {
                return redirect('user/add')->withInput($request->all())->with([
                    'message' => 'Insert valid sms balance',
                    'message_important' => true
                ]);
            }
        }
    ...
    public function viewUser($id)
    {
        ...
        $client = Client::where('parent', auth('team')->user()->cl_id)->find($id);
        if ($client) {
            ...
            return view('client.user-manage', compact('client', 'invoices', 'tickets', 'sms_gateways', 'sms_transaction', 'clientGroups', 'selected_gateways', 'sender_ids'));
        } else {
            return redirect('user/all')->with([
    ...
    public function updateLimit(Request $request)
    {
        ...
        $client = Client::where('parent', auth('team')->user()->cl_id)->find($cmd);
        if ($client && $parent) {

            $client_balance = $client->sms_limit;

            if ($client_balance == 0 && $request->sms_amount  < 0) {
                return redirect('user/view/' . $cmd)->with([
                    'message' => 'Insert positive or valid sms balance',
                    'message_important' => true
                ]);
            }

            $remaining_balance = $client_balance + $request->sms_amount;

            if ($request->sms_amount > $parent->sms_limit || $remaining_balance < 0) {
                return redirect('user/view/' . $cmd)->with([
                    'message' => 'You do not have enough sms balance',
                    'message_important' => true
                ]);
    ...
    public function sendSMS(Request $request)
    {
        ...
        $account = Client::find(auth('team')->user()->cl_id);
        ...
        if ($sms_cost) {
            ...
            if ($sms_charge > $account->sms_limit) {
                return redirect('user/view/' . $cmd)->withInput($request->all())->with([
                    'message' => 'You do not have enough sms balance',
                    'message_important' => true
                ]);
        ...
        $this->dispatch(new SendBulkSMS(auth('team')->user()->cl_id, $client->phone, $gateway, $gateway_credential, $sender_id, $message, $msgcount, $cg_info, '', $msg_type));

        $remain_sms        = $account->sms_limit - $sms_charge;
        $account->sms_limit = $remain_sms;
        $account->save();

        return redirect('user/view/' . $cmd)->with([
            'message' => 'Please check sms history'
        ]);
    }
    ...
    public function exportImport()
    {
        ...
        $client_groups = ClientGroups::where('status', 'Yes')->where('created_by', auth('team')->user()->cl_id)->get();
    ...
    public function exportUsers()
    {
        ...
        Excel::create('Clients', function ($excel) {
            $excel->sheet('Sheetname', function ($sheet) {
                $sheet->fromModel(Client::where('parent', auth('team')->user()->cl_id)->get());
            });
        })->export('csv');
    }
    ...
    public function userGroups()
    {
        ...
        $clientGroups = ClientGroups::where('created_by', auth('team')->user()->cl_id)->get();
    ...
    public function addNewUserGroup(Request $request)
    {
        ...
        $clientGroup->created_by = auth('team')->user()->cl_id;
    ...
    public function updateUserGroup(Request $request)
    {
        ...
        $clientGroup = ClientGroups::where('created_by', auth('team')->user()->cl_id)->find($cmd);
    ...
    public function deleteUserGroup($id)
    {
        ...
        $clientGroup = ClientGroups::where('created_by', auth('team')->user()->cl_id)->find($id);

        if ($clientGroup) {
            $exist_client = Client::where('groupid', $id)->first();
            if ($exist_client) {
                return redirect('users/groups')->with([
                    'message' => 'This Group exist in a client',
                    'message_important' => true
                ]);
            }
    ...
    public function deleteUser($id)
    {
        ...
        if ($client) {
            ...
            if ($client_group) {
                return redirect('user/all')->with([
                    'message' => 'Client contain in' . ' ' . 'Client Group',
                    'message_important' => true
                ]);
            }
            ...
            if ($client_inv) {
                return redirect('user/all')->with([
                    'message' => 'Client contain in' . ' ' . 'Invoice',
                    'message_important' => true
                ]);
            }
            ...
            if ($client_sms) {
                return redirect('user/all')->with([
                    'message' => 'Client contain in' . ' ' . 'SMS History',
                    'message_important' => true
                ]);
            }
            ...
            if ($client_sender_id) {
                return redirect('user/all')->with([
                    'message' => 'Client contain in' . ' ' . 'Sender ID',
                    'message_important' => true
                ]);
            }
            ...
            if ($client->sms_limit > 0) {
                return redirect('user/all')->with([
                    'message' => 'Client sms limit not empty',
                    'message_important' => true
                ]);
            }
            ...
            if ($parents) {
                return redirect('user/all')->with([
                    'message' => 'This client have some customer',
                    'message_important' => true
                ]);
            }
            ...
            if ($client_tickets) {
                return redirect('user/all')->with([
                    'message' => 'Client contain in' . ' ' . 'Support Tickets',
                    'message_important' => true
                ]);
            }
    ...
    public function getAllClients()
    {
        ...
        $clients = Client::where('parent', auth('team')->user()->cl_id)->orderBy('datecreated', 'desc');
        return Datatables::of($clients)
            ->addColumn('action', function ($cl) {
                return '
                <a class="btn btn-success btn-xs" href="' . url("user/view/$cl->id") . '" ><i class="fa fa-edit"></i>' . 'Manage' . '</a>
                <a href="#" id="' . $cl->id . '" class="cdelete btn btn-xs btn-danger"><i class="fa fa-danger"></i> ' . 'Delete' . '</a>
                ';
            })
