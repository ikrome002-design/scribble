...
class UserSMSController extends Controller
{
    ...
    public function postSenderID(Request $request)
    {
        $this->validate(
            ...
            [
                'sender_id' => 'required',
                'files.*' => 'required|mimes:png,jpg,jpeg,docx,doc,csv,txt,xlx,xls,xlsx,pdf,zip|max:50000000'
            ]
        );
        ...
        $client_id = (string)auth('team')->user()->cl_id;
        $client_id = (array)$client_id;
        $client_id = json_encode($client_id);

        $sender = new  SenderIdManage();
        $sender->sender_id = $request->sender_id;
        $sender->cl_id = $client_id;
        ...
        if ($files) {
            foreach ($files as $file) {
                ...
                $filename = basename($disk->putFile('/senderid', $file));
            ...
            SenderIdFiles::insert($store_files);
        }
        ...
        $url = url('sms/view-sender-id/' . $sender->id);

        $client = Client::find(auth('team')->user()->cl_id);

        try {
            \Mail::to(app_config('Email'))->send(new RequestSenderID(auth('team')->user()->client->fname, $client->email, $request->sender_id, $url, $files_email));
    ...
    public function sendBulkSMS()
    {
        ...
        $phone_book = ImportPhoneNumber::where('user_id', auth('team')->user()->cl_id)->get();
        $client_group = ClientGroups::where('created_by', auth('team')->user()->cl_id)->where('status', 'Yes')->get();
        $sms_templates = SMSTemplates::where('status', 'active')->where('cl_id', auth('team')->user()->cl_id)->orWhere('global', 'yes')->get();
        ...
        $keyword = Keywords::where('user_id', auth('team')->user()->cl_id)->where('status', 'assigned')->get();
        $schedule_sms = false;
    ...
    public function postSendBulkSMS(Request $request)
    {
        ...
        if ($gateway->status != 'Active') {
            return redirect($redirect_url)->withInput($request->all())->with([
                'message' => 'SMS gateway not active.Contact with Provider',
                'message_important' => true
            ]);
        }
        ...
        if ($msg_type == 'voice') {
            if ($gateway->voice != 'Yes') {
                return redirect($redirect_url)->withInput($request->all())->with([
                    'message' => 'SMS Gateway not supported Voice feature',
                    'message_important' => true
                ]);
            }
        }
        ...
        if ($msg_type == 'mms') {
            ...
            if ($gateway->mms != 'Yes') {
                return redirect($redirect_url)->withInput($request->all())->with([
                    'message' => 'SMS Gateway not supported MMS feature',
                    'message_important' => true
                ]);
            }
            ...
            if (app_config('AppStage') != 'Demo') {
                if (isset($image) && in_array(strtolower($image->getClientOriginalExtension()), array("png", "jpeg", "gif", 'jpg', 'mp3', 'mp4', '3gp', 'mpg', 'mpeg'))) {
                    ...
                    return redirect($redirect_url)->withInput($request->all())->with([
                        'message' => 'Upload .png or .jpeg or .jpg or .gif or .mp3 or .mp4 or .3gp or .mpg or .mpeg file',
                        'message_important' => true
                    ]);
                }
            } else {
                return redirect($redirect_url)->withInput($request->all())->with([
                    'message' => 'MMS is disable in demo mode',
                    'message_important' => true
                ]);
            }
        } else {
        ...
        if (isset($results) && is_array($results)) {
            ...
            if (count($results) > 0) {
                ...
                $filtered_data = [];
                $blacklist = BlackListContact::select('numbers')->where('user_id', auth('team')->user()->cl_id)->get()->toArray();
                ...
                foreach (array_chunk($results, 50) as $chunk_result) {
                    foreach ($chunk_result as $r) {
                        ...
                        if ($total_cost == 0) {
                            return redirect($redirect_url)->withInput($request->all())->with([
                                'message' => 'You do not have enough sms balance',
                                'message_important' => true
                            ]);
                        }
                ...
                if (isset($final_insert_data) && is_array($final_insert_data) && count($final_insert_data) > 0) {
                    ...
                    if ($campaign) {
                        ...
                        if (app_config('fraud_detection') == 1) {
                            if (isset($block_message) && is_array($block_message) && count($block_message) > 0) {
                                ...
                                if ($store_block_message) {
                                    ...
                                    $sysUrl = url('clients/view/' . auth('team')->user()->cl_id);
                                    $user_name = auth('team')->user()->client->username;
                                    $client_email = auth('team')->user()->client->email;
                        ...
                        if ($campaign_list) {
                            ...
                            return redirect($redirect_url)->with([
                                'message' => 'SMS added in queue and will deliver one by one'
                            ]);
                        }
    ...
    public function purchaseSMSPlan()
    {
        ...
        // $price_plan =  SMSPricePlan::where(['client_id' => $user_id, 'show_client' => 'Yes', 'sys_sms_price_plan.status' => 'Active'])
        //     ->join('sys_clients', 'sys_clients.id', '=', 'sys_sms_price_plan.client_id')
        //     ->join('sys_client_groups', 'sys_client_groups.id', '=', 'sys_sms_price_plan.client_group_id')
        //     ->join('sys_plan_name', 'sys_plan_name.id', '=', 'sys_sms_price_plan.plan_id')
        //     ->select('sys_sms_price_plan.*', 'sys_clients.fname', 'sys_clients.lname', 'sys_client_groups.group_name', 'sys_plan_name.plan_name')
        //     ->get();
        ...
        $price_plan =  SMSPricePlan::where('client_id', $user_id)->get();
        $payment_gateways = PaymentGateways::where('status', 'Active')->get();
        return view('client.buy-unit-plan', compact('payment_gateways', 'price_plan'));
    }

    // this method is use to buy the range
    public function buyRange(Request $request)
    {
        $buy_unit = $request->buy_unit;
        $user_id = $request->user_id;

        $sms_price_plan = SMSPricePlan::where('client_id', $user_id)->get();

        // echo "<pre>";
        // print_r($sms_price_plan);
        // die;

        if (isset($sms_price_plan) && count($sms_price_plan) > 0) {

            $data = [];
            foreach ($sms_price_plan as $sms_price_plans) {

                if ($buy_unit > $sms_price_plans->unit_from && $buy_unit < $sms_price_plans->unit_to) {

                    // dd('its coming');

                    $data['unit_price'] = $sms_price_plans->price;
                    $data['amt_pay'] = $buy_unit * $sms_price_plans->price;
                    $data['trans_fee'] = $sms_price_plans->transaction_fee . " %";

                    // apply trans fee on amt                    
                    $appl_trans_fee = $data['amt_pay'] * $sms_price_plans->transaction_fee / 100;

                    // amount after transaction fee
                    $amt_trns_fee = $data['amt_pay'] + $appl_trans_fee;

                    if ($sms_price_plans->discount_type == 3) {
                        $dicount = $sms_price_plans->discount_amount . " KES";
                        $amt_disc_fee = $amt_trns_fee - $sms_price_plans->discount_amount;
                    } elseif ($sms_price_plans->discount_type == 2) {
                        $dicount = $sms_price_plans->discount_amount . " KES";
                        $amt_disc_fee = $amt_trns_fee - $sms_price_plans->discount_amount;
                    } else {
                        $dicount = $sms_price_plans->discount_amount . " %";
                        $per_disc = $amt_trns_fee *  $sms_price_plans->discount_amount / 100;

                        $amt_disc_fee = $amt_trns_fee - $per_disc;
                    }

                    $data['discount'] = $dicount;

                    if ($sms_price_plans->govt_charges_type == 2) {
                        $govt_charge = $sms_price_plans->govt_charges_amt . " KES";
                        $up_govt = $sms_price_plans->govt_charges_amt;
                        $amt_gov_fee  =  $amt_disc_fee + $up_govt;
                    } else {
                        $govt_charge = $sms_price_plans->govt_charges_amt . " %";
                        $up_govt = $sms_price_plans->govt_charges_amt;
                        $per_govt = $amt_disc_fee * $up_govt / 100;
                        $amt_gov_fee = $amt_disc_fee + $per_govt;
                    }

                    $data['govt_charge'] = $govt_charge;
                    $data['Total'] = $amt_gov_fee . ' KES';
                    ...
                    $status = false;
                    $data = [];
                    $message = "The number of SMS units requested is not currently on offer. Kindly contact the Admin via Support Ticket for further clarification.";
                }
            }
        } else {
            $status = false;
            $data = [];
            $message = 'sms price plan not found';
        }
    ...
    public function viewPricePlan($id)
    {
        $user_id = auth('team')->user()->cl_id;
        ...
        return view('client.view-sms-price-plan', compact('price_plan'));
    }

    //airtime bundles

    public function PurchaseAirtimeBundle()
    {
        ...
        if (!$client) {
            return back()->withErrors("You don't have an active plan.");
        }

        $airtime_bundle =  AirtimeBundle::where('status', 'Active')
            ->where('plan_id', $client->plan_id)
            ->where(function ($query) use ($client) {
                $query->where('client_group_id', '=', 0)
                    ->orWhere('client_group_id', '=', null)
                    ->orWhere('client_group_id', '=', $client->groupid);
            })
            ->where(function ($query) use ($client) {
        ...
        return view('client.all-artime-bundles', compact('airtime_bundle'));
    }


    public function viewAirtimeBundle($id)
    {
        ...
        $price_plan =  AirtimeBundle::where('id', $id)
            ->where('plan_id', $client->plan_id)
            ->where(function ($query) use ($client) {
                $query->where('client_group_id', '=', 0)
                    ->orWhere('client_group_id', '=', null)
        ...
        if (!$price_plan) {
            return redirect('user/sms/purchase-airtime-bundle')->with('message', 'Bundle Not found');
        }

        $invoice = new PaymentInvoiceController();
        $items[] = ['plan' => $price_plan, 'quantity' => $price_plan->units];

        $invoice_no = $invoice->generateInvoice($client, 'Single', $items);

        return redirect('user/invoices/view/' . $invoice_no)
            ->with('message', 'Please pay this invoice before due date');


        // $url = "/user/sms/airtime-bundle/calculate/" . $id;
        // return view('client.buy-unit-plan', compact('price_plan', 'url'));
    }

    public function artimeBundleCalc(Request $request, $id)
    {
        ...
        $airtime =  AirtimeBundle::where('id', $id)
            ->where('plan_id', $client->plan_id)
            ->where(function ($query) use ($client) {
                $query->where('client_group_id', '=', 0)
                    ->orWhere('client_group_id', '=', null)
        ...
        if (!$airtime || $units > $airtime->unit_to  || $units < $airtime->unit_from) {
            return [false];
        };
    ...
    public function allSmsPricePlan(Request $request)
    {
        ...
        $smsPlans =  SMSPricePlan::where('status', 'Active')
            ->where('plan_id', $client->plan_id)
            ->where(function ($query) use ($client) {
                $query->where('client_group_id', '=', 0)
                    ->orWhere('client_group_id', '=', null)
        ...
        if ($request->isMethod('post')) {
            ...
            $units = round($request->units);

            $client = Client::where('id', auth('team')->user()->cl_id)
                ->where('plan_status', 'Active')->first();

            $price_plan = SMSPricePlan::where('unit_from', '<=', $request->units)
                ->where('unit_to', '>=', $request->units)
                ->where('plan_id', $client->plan_id)
                ->where(function ($query) use ($client) {
                    $query->where('client_group_id', '=', 0)
                        ->orWhere('client_group_id', '=', null)
            ...
            if (!$price_plan) {
                return back()->withErrors("The units doesn't match with any sms plan");
            }

            $invoice = new PaymentInvoiceController();
            $items[] = ['plan' => $price_plan, 'quantity' => $units, 'description' => "SMS Plan purchase"];
            $invoice_no = $invoice->generateInvoice(client: $client, invoice_type: 'Single', items: $items, sms_quantity: $request->units);

            return redirect('user/invoices/view/' . $invoice_no)
                ->with('message', 'Please pay this invoice before due date');
        }
        return view('client.all-sms-price-plans', compact('smsPlans'));
    }


    public function viewSmsPricePlan(Request $request, $id)
    {
        ...
        $price_plan =  SMSPricePlan::where('id', $id)
            ->where('plan_id', $client->plan_id)
            ->where(function ($query) use ($client) {
                $query->where('client_group_id', '=', 0)
                    ->orWhere('client_group_id', '=', null)
        ...
        if (!$price_plan) {
            return redirect('user/sms/purchase-sms-plan')->with('message', 'Bundle Not found');
        }
        $maxUnits = $price_plan->unit_to;
        $minUnits = $price_plan->unit_from;
        if ($request->isMethod('post')) {
            $this->validate($request, [
                'units' => "required|integer|min:$minUnits|max:$maxUnits"
            ]);
        ...
        $url = "/user/sms/sms-price-plan/calculate/" . $id;
        return view('client.buy-unit-plan', compact('price_plan', 'url'));
    }

    public function smsPricePlanCalc(Request $request)
    {
        ...
        $smsPlan = SMSPricePlan::where('unit_from', '<=', $request->units)
            ->where('unit_to', '>=', $request->units)
            ->where('plan_id', $client->plan_id)
            ->where(function ($query) use ($client) {
                $query->where('client_group_id', '=', 0)
        ...
        if (!$smsPlan) {
            return ['error' => "Units don't fall in range of sms plans"];
        }


        $calc = new PriceCalculation();
        $calc = $calc->smsPriceCalculation($client, $smsPlan, $units);
        return  $calc;
    }
    ...
    public function postSMSFromFile(Request $request)
    {
        ...
        if (!in_array_r(strtolower($file_extension), $supportedExt)) {
            return redirect($redirect_url)->withInput($request->all())->with([
                'message' => 'Insert Valid Excel or CSV file',
                'message_important' => true
            ]);
        }
        ...
        $all_data = Excel::toArray([], $request->import_numbers);
    ...
    public function smsViewInbox($id)
    {
        ...
        $inbox_info = SMSHistory::where('userid', auth('team')->user()->cl_id)->find($id);
    ...
    public function deleteSMS($id)
    {
        ...
        if ($inbox_info) {
            ...
            return redirect('user/sms/history')->with([
                'message' => 'SMS info deleted successfully'
            ]);
        } else {
            return redirect('user/sms/history')->with([
                'message' => 'SMS Not Found',
    ...
    public function updateApiInfo(Request $request)
    {
        ...
        $client = Client::find(auth('team')->user()->cl_id);
        $client->api_gateway = $request->sms_gateway;
        if ($request->generate_new_api_key) {
            if (!Hash::check($request->password, $client->password) || !$request->password) {
                return back()->withErrors('Wrong password');
            }
            $client->api_key = bin2hex(random_bytes(32));
        }
        $client->save();
    ...
    public function getAllScheduleSMS()
    {
        ...
        $schedule_sms = ScheduleSMS::where('userid', auth('team')->user()->cl_id)->select(['id', 'sender', 'receiver', 'submit_time']);

        return Datatables::of($schedule_sms)
            ->addColumn('action', function ($ss) {
                return "
               <a class='btn btn-success btn-xs' href='" . url('user/sms/manage-update-schedule-sms/$ss->id') . "'><i class='fa fa-edit'></i>Edit</a>
               <a href='#' class='btn btn-danger btn-xs cdelete' id='$ss->id'><i class='fa fa-trash'></i>Delete</a>";
            })
        ...
        if ($request->has('data_ids')) {
            ...
            if (isset($all_ids) && is_array($all_ids) && count($all_ids) > 0) {
                foreach ($all_ids as $id) {
                    $sms = ScheduleSMS::where('userid', auth('team')->user()->cl_id)->find($id);
                    if ($sms) {
    ...
    public function postUpdateScheduleSMS(Request $request)
    {
        ...
        if ($sms_info->amount < $msgcount) {
            ...
            if ($sms_cost) {
                ...
                return redirect('user/sms/manage-update-schedule-sms/' . $cmd)->withInput($request->all())->with([
                    'message' => 'Phone Number Coverage are not active',
                    'message_important' => true
                ]);
            }
        } else {
            ...
            if (isset($spam_word) && is_array($spam_word) && count($spam_word) > 0) {
                ...
                if (isset($check) && is_array($check) && count($check) > 0) {
                    ...
                    return redirect('user/sms/update-schedule-sms')->with([
                        'message' => 'SMS are scheduled. Deliver in correct time'
                    ]);
                }
            }
        }
    ...
    public function buyUnitPlan($id)
    {

        $price_plan =  SMSPricePlan::where('id', $id)->first();
        // dd($price_plan);
        $payment_gateways = PaymentGateways::where('status', 'Active')->get();
        return view('client.buy-unit-plan', compact('payment_gateways', 'price_plan'));
    }
    ...
    {
        $sms_templates = SMSTemplates::where('cl_id', auth('team')->user()->cl_id)->orWhere('global', 'yes')->get();
        ...
        if (app_config('sender_id_verification') == '1') {
            ...
            foreach ($all_sender_id as $sid) {
                ...
                if (in_array('0', $client_array)) {
                    array_push($all_ids, $sid->sender_id);
                } elseif (in_array(auth('team')->user()->cl_id, $client_array)) {
                    array_push($all_ids, $sid->sender_id);
                }
            }
            $sender_ids = array_unique($all_ids);
    ...
    public function postSmsTemplate(Request $request)
    {
        ...
        $exist = SMSTemplates::where('template_name', $request->template_name)->where('cl_id', auth('team')->user()->cl_id)->first();
    ...
    public function postBlacklistContact(Request $request)
    {
        ...
        try {
            ...
            foreach ($results as $r) {
                ...
                $exist = BlackListContact::where('numbers', $phone)->where('user_id', auth('team')->user()->cl_id)->first();
    ...
    public function deleteBlacklistContact($id)
    {
        ...
        $blacklist = BlackListContact::where('user_id', auth('team')->user()->cl_id)->find($id);
        if ($blacklist) {
            $blacklist->delete();
            return redirect('user/sms/blacklist-contacts')->with([
    ...
    public function deleteBulkBlacklistContact(Request $request)
    {
        ...
        if ($request->has('data_ids')) {
            ...
            if (isset($all_ids) && is_array($all_ids) && count($all_ids) > 0) {
                $status = BlackListContact::where('user_id'->whereIn('id', $all_ids))->delete();
    ...
    {
        $blacklist = BlackListContact::select(['id', 'numbers'])->where('user_id', auth('team')->user()->cl_id)->get();
        return Datatables::of($blacklist)
            ->addColumn('action', function ($bl) {
                return '
            <a href="#" class="btn btn-danger btn-xs cdelete" id="' . $bl->id . '"><i class="fa fa-trash"></i>Delete</a>';
            })
    ...
    public function getSmsHistoryData(Request $request)
    {
        ...
        if ($request->order && $request->columns) {
        ...
        $sms_history = SMSHistory::where('userid', auth('team')->user()->cl_id)
            ->orderBy($get_search_column, $short_by);
        return Datatables::of($sms_history)
            ->addColumn('action', function ($sms) {
                return '
                <a class="btn btn-success btn-xs" href="' . url("user/sms/view-inbox/$sms->id") . '" ><i class="fa fa-inbox"></i> ' . 'Inbox' . '</a>
                <a href="#" id="' . $sms->id . '" class="cdelete btn btn-xs btn-danger"><i class="fa fa-danger"></i> ' . 'Delete' . '</a>
                ';
            })
            ...
            ->filter(function ($query) use ($request) {
                ...
                if ($request->send_by && $request->send_by != '0') {
            ...
            ->addColumn('send_by', function ($sms) {
                if ($sms->send_by == 'api') {
                    return '<span class="text-info">API SMS</span>';
                } else {
                    return 'Outgoing';
                }
            })
    ...
    public function deleteBulkSMS(Request $request)
    {
        ...
        if ($request->has('data_ids')) {
            ...
            if (is_array($all_ids) && count($all_ids) > 0) {
                SMSHistory::where('userid', auth('team')->user()->cl_id)->whereIn('id', $all_ids)->delete();
            }
        }
    }
            ...
            if (isset($spam_word) && is_array($spam_word) && count($spam_word) > 0) {
                ...
                if (isset($check) && is_array($check) && count($check) > 0) {
                    return redirect('user/sms/quick-sms')->with([
                        'message' => 'Your are sending fraud message',
                        'message_important' => true
                    ]);
                }
            }
        ...
        if ($h) {
            ...
            if (app_config('fraud_detection') == 1) {
                ...
                if (isset($spam_word) && is_array($spam_word) && count($spam_word) > 0) {
                    ...
                    if (isset($check) && is_array($check) && count($check) > 0) {
                        return redirect('user/sms/history')->with([
                            'message' => 'Message contain spam word',
                            'message_important' => true
                        ]);
                    }
                }
            ...
            if ($msg_type == 'mms') {
                return redirect('user/sms/history')->with([
                    'message' => 'MMS not supported in two way communication',
                    'message_important' => true
                ]);
            }
    ...
    */
    ...
    public function getRecurringSMSData(Request $request)
    {
        ...
        $recurring_sms = RecurringSMS::select(['id', 'sender', 'status', 'total_recipients', 'recurring_date', 'recurring', 'updated_at'])->where('userid', auth('team')->user()->cl_id)->orderBy($get_search_column, $short_by);
        return Datatables::of($recurring_sms)
            ->addColumn('action', function ($sms) {
                $reply_url = '';
                if ($sms->status == 'running') {
                    $reply_url .= ' <a class="btn btn-warning btn-xs stop-recurring" href="#" id="' . $sms->id . '"><i class="fa fa-stop"></i> ' . 'Stop Recurring' . '  </a>';
                } else {
                    $reply_url .= ' <a class="btn btn-success btn-xs start-recurring" href="#" id="' . $sms->id . '"><i class="fa fa-check"></i> ' . 'Start Recurring' . ' </a>';
                }
                return $reply_url . '
                <a href="#" id="' . $sms->id . '" class="cdelete btn btn-xs btn-danger"><i class="fa fa-trash"></i> Delete</a>
                <div class="btn-group btn-mini-group dropdown-default">
                    ...
                    <ul class="dropdown-menu">
                        <li><a href="' . url("user/sms/update-recurring-sms/$sms->id") . '" data-toggle="tooltip" data-placement="left" title="' . 'Update Period' . '"><i class="fa fa-clock-o"></i></a></li>
                        <li><a href="' . url("user/sms/add-recurring-sms-contact/$sms->id") . '" data-toggle="tooltip" data-placement="left" title="' . 'Add Contact' . '"><i class="fa fa-plus"></i></a></li>
                        <li><a href="' . url("user/sms/update-recurring-sms-contact/$sms->id") . '" data-toggle="tooltip" data-placement="left" title="' . 'Update Contact' . '"><i class="fa fa-edit"></i></a></li>
                    </ul>
                </div>
                ';
            })
            ...
            ->addColumn('status', function ($sms) {
                if ($sms->status == 'running') {
                    ...
                    return '<span class="text-danger"> Stop</span>';
                }
            })
    ...
    public function deleteRecurringSMS($id)
    {
        ...
        $recurring_sms = RecurringSMS::where('userid', auth('team')->user()->cl_id)->find($id);
    ...
    public function deleteRecurringSMSContact($id)
    {
        ...
        if ($recurring_contact) {
            $check_exist = RecurringSMS::where('userid', auth('team')->user()->cl_id)->find($recurring_contact->campaign_id);
            if ($check_exist) {
        ...
        if ($request->has('data_ids')) {
            ...
            if (is_array($all_ids) && count($all_ids) > 0) {
                foreach ($all_ids as $id) {
                    ...
                    $check_exist = RecurringSMS::where('userid', auth('team')->user()->cl_id)->find($id);
    ...
    public function bulkDeleteRecurringSMSContact(Request $request)
    {
        ...
        if ($request->has('data_ids')) {
            ...
            if ($request->has('campaign_id')) {
                if (is_array($all_ids) && count($all_ids) > 0) {
                    $recurring = RecurringSMS::where('userid', auth('team')->user()->cl_id)->find($request->campaign_id);
                    if ($recurring) {
                        ...
                        return response()->json([
                            'status' => 'error',
                            'message' => 'Recurring SMS info not found'
                        ]);
                    }
                } else {
                    return response()->json([
        ...
        if (isset($results) && is_array($results)) {
            ...
            if (count($results) > 0) {
                ...
                if ($recurring_id) {
                    ...
                    return redirect('user/sms/send-recurring-sms')->with([
                        'message' => 'Message recurred successfully. Delivered in correct time'
                    ]);
                } else {
                    return redirect('user/sms/send-recurring-sms')->with([
                        'message' => 'Something went wrong please try again',
    ...
    {
        $recurring = RecurringSMS::where('userid', auth('team')->user()->cl_id)->find($id);
        ...
        if ($recurring) {
            ...
            return redirect('user/sms/recurring-sms')->with([
                'message' => 'Recurring SMS stop successfully'
            ]);
        } else {
            return redirect('user/sms/recurring-sms')->with([
                'message' => 'Recurring information not found',
                'message_important' => true
            ]);
        }
    }
    ...
    public function startRecurringSMS($id)
    {
        ...
        if ($recurring) {
            ...
            return redirect('user/sms/recurring-sms')->with([
                'message' => 'Recurring SMS running successfully'
            ]);
        } else {
            return redirect('user/sms/recurring-sms')->with([
                'message' => 'Recurring information not found',
    ...
    public function postUpdateRecurringSMS(Request $request)
    {
        ...
        $recurring = RecurringSMS::where('userid', auth('team')->user()->cl_id)->find($cmd);
        ...
        if ($recurring) {
            ...
            return redirect('user/sms/recurring-sms')->with([
                'message' => 'Recurring SMS period changed'
            ]);
        } else {
            return redirect('user/sms/recurring-sms')->with([
                'message' => 'Recurring information not found',
    ...
    public function postRecurringSMSContact(Request $request)
    {
        ...
        if ($recurring) {
            ...
            if (isset($results) && is_array($results)) {
                ...
                return redirect('user/sms/add-recurring-sms-contact/' . $id)->with([
                    'message' => 'Recurring contact added successfully'
                ]);
            } else {
    ...
    public function postUpdateRecurringSMSContactData(Request $request)
    {
        ...
        if ($recurring) {
            ...
            return redirect('user/sms/update-recurring-sms-contact-data/' . $contact_id)->with([
                'message' => 'Recurring contact updated successfully'
            ]);
        } else {
            return redirect('user/sms/recurring-sms')->with([
                'message' => 'Recurring information not found',
    ...
    public function getRecurringSMSContactData($id, Request $request)
    {
        ...
        return Datatables::of($recurring_sms)
            ->addColumn('action', function ($sms) {
                return '
                <a href="' . url("user/sms/update-recurring-sms-contact-data/$sms->id") . '" class="btn btn-xs btn-complete"><i class="fa fa-edit"></i>Update</a>
                <a href="#" id="' . $sms->id . '" class="cdelete btn btn-xs btn-danger"><i class="fa fa-trash"></i> Delete</a>
    ...
    public function getAllKeywords()
    {
        ...
        $keywords = Keywords::select(['id', 'user_id', 'title', 'keyword_name', 'status', 'price', 'reply_mms', 'validity'])->where('status', 'available')->orWhere('user_id', auth('team')->user()->cl_id);
        return Datatables::of($keywords)
            ->addColumn('action', function ($kw) {

                $reply_url = '';

                if ($kw->status == 'available' && $kw->user_id != auth('team')->user()->cl_id) {
                    $reply_url .= '
                <a class="btn btn-success btn-xs" href="' . url("user/keywords/purchase/$kw->id") . '" ><i class="fa fa-shopping-cart"></i> Purchase Now</a>
                ';
                }

                if ($kw->user_id == auth('team')->user()->cl_id) {
                    if ($kw->reply_mms) {
                        $reply_url .= ' <a href="#" id="id_' . $kw->id . '" class="remove_mms btn btn-xs btn-primary"><i class="fa fa-remove"></i> Remove MMS</a>';
                    }

                    $reply_url .= '
                <a class="btn btn-success btn-xs" href="' . url("user/keywords/view/$kw->id") . '" ><i class="fa fa-edit"></i> Manage</a>
                ';
                }
    ...
    {
        $keyword = Keywords::where('user_id', auth('team')->user()->cl_id)->find($id);
        if ($keyword) {
    ...
    public function postManageKeyword(Request $request)
    {
        ...
        $keyword = Keywords::where('user_id', auth('team')->user()->cl_id)->find($keyword_id);
    ...
    public function removeKeywordMMSFile($id)
    {
        ...
        if (isset($keyword_id) && is_array($keyword_id) && array_key_exists('1', $keyword_id)) {
            $keyword = Keywords::where('user_id', auth('team')->user()->cl_id)->find($keyword_id['1']);
    ...
    public function getCampaignReports(Request $request)
    {
        ...
        $campaigns = Campaigns::orderBy($get_search_column, $short_by)->where('user_id', auth('team')->user()->cl_id);
        return Datatables::of($campaigns)
            ->addColumn('action', function ($campaign) {
                return '
                <a class="btn btn-success btn-xs" href="' . url("user/sms/manage-campaign/$campaign->id") . '" ><i class="fa fa-line-chart"></i> Reports</a>
                <a href="#" id="' . $campaign->id . '" class="cdelete btn btn-xs btn-danger"><i class="fa fa-trash"></i> Delete</a>
                ';
            })
            ...
            ->filter(function ($query) use ($request) {
                ...
                if ($request->camp_type && $request->camp_type != '0') {
                    $query->where('camp_type', $request->camp_type);
                }
    ...
    public function manageCampaign($id)
    {
        ...
        $campaign = Campaigns::where('user_id', auth('team')->user()->cl_id)->find($id);
        ...
        if ($campaign) {
            ...
            $keyword = Keywords::where('user_id', auth('team')->user()->cl_id)->get();
        ...
        return Datatables::of($campaign_list)
            ->addColumn('action', function ($campaign) {
                ...
                if ($campaign->submitted_time != null && $campaign->status == 'scheduled' && new \DateTime() < new \DateTime($campaign->submitted_time)) {
                    $url .= '
                <a class="btn btn-success btn-xs" href="' . url("user/sms/manage-update-schedule-sms/$campaign->id") . '" ><i class="fa fa-clock-o"></i> Manage</a>
                ';
                }
    ...
    public function deleteBulkCampaignRecipients(Request $request)
    {
        ...
        if ($request->has('data_ids')) {
            ...
            if ($request->has('campaign_id')) {
                ...
                $campaign = Campaigns::where('user_id', auth('team')->user()->cl_id)->find($request->campaign_id);
    ...
    public function deleteCampaignRecipient($id)
    {
        ...
        $recipient = CampaignSubscriptionList::find($id);
        if ($recipient) {
            $campaign = Campaigns::where('campaign_id', $recipient->campaign_id)->where('user_id', auth('team')->user()->cl_id)->first();
            if ($campaign) {
    ...
    {
        $get_data = SMSHistory::where('userid', auth('team')->user()->cl_id)->where('send_by', 'receiver')->orderBy('updated_at', 'DESC');
    ...
    public function addToBlacklist(Request $request)
    {
        ...
        $inbox_info = SMSHistory::where('userid', auth('team')->user()->cl_id)->find($request->sms_id);

        if ($inbox_info) {

            $phone = $inbox_info->receiver;

            $blacklist = BlackListContact::where('numbers', $phone)->where('user_id', auth('team')->user()->cl_id)->first();
    ...
    public function replyChatSMS(Request $request)
    {
        ...
        $h = SMSHistory::where('userid', auth('team')->user()->cl_id)->find($cmd);
        ...
        if ($h) {
            ...
            if ($gateway->custom == 'Yes') {
                if ($gateway->type == 'smpp') {
                    ...
                    if ($gateway_credential == null) {
                        return response()->json([
                            'status' => 'error',
                            'message' => 'SMS Gateway credential not found'
                        ]);
                    }
                } else {
                    $cg_info = CustomSMSGateways::where('gateway_id', $gateway->id)->first();
            ...
            $blacklist = BlackListContact::where('numbers', $h->sender)->where('user_id', auth('team')->user()->cl_id)->first();
    ...
    public function removeChatHistory(Request $request)
    {
        ...
        $sms_history = SMSHistory::where('userid', auth('team')->user()->cl_id)->find($request->sms_id);
        if ($sms_history) {
