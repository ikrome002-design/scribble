...
@section('content')
    <section class="wrapper-bottom-sec">
        <div class="p-30 clearfix">
            ...
            <a href="{{ url('sms/add-sender-id') }}" class="btn btn-success btn-sm pull-right"><i class="fa fa-plus"></i>
                {{ language_data('Add Sender ID') }}</a>
        </div>
                                        ...
                                        <tr>
                                            ...
                                            @if ($si->status == 'unblock')
                                                <td data-label="Status">
                                                    <p class="label label-success">{{ language_data('Unblock') }}</p>
                                                </td>
                                            @elseif($si->status == 'block')
                                                <td data-label="Status">
                                                    <p class="label label-danger">{{ language_data('Block') }}</p>
                                                </td>
                                            @else
                                                <td data-label="Status">
                                                    <p class="label label-warning">{{ language_data('Pending') }}</p>
                                                </td>
                                            @endif

                                            <td data-label="Actions">
                                                <a class="btn btn-success btn-xs"
                                                    href="{{ url('sms/view-sender-id/' . $si->id) }}"><i
                                                        class="fa fa-edit"></i> {{ language_data('Manage') }}</a>
                                                <a href="#" class="btn btn-danger btn-xs cdelete"
                                                    id="{{ $si->id }}"><i class="fa fa-trash"></i>
