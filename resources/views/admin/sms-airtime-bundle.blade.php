                ...
                <div class="col-lg-12">
                    <div class="panel">
                        ...
                        <a class="btn btn-success btn-xs" style="margin-bottom : 6px; margin-left : 6px;"
                            href="{{ url('sms/add-airtime-bundle') }}"><i class="fa fa-plus"></i>
                            {{ language_data('Add Airtime Bundle') }}</a>
                                    ...
                                    @foreach ($airtime_bundle as $airtime)
                                        <tr>
                                            ...
                                            <td data-label="govt_charges_amt">
                                                <p>{{ $airtime->govt_charges_amt }}</p>
                                            </td>
                                            <td data-label="client_group">
                                                @if (!$airtime->client_group_id)
                                                    <p>All</p>
                                                @else
                                                    <p>{{ $airtime->clientGroup->group_name }}</p>
                                                @endif
                                            </td>
                                            ...
                                            <td data-label="Actions">
                                                <a class="btn btn-complete btn-xs"
                                                    href="{{ url('sms/edit-airtime-bundle/' . $airtime->id) }}"><i
                                                        class="fa fa-edit"></i> {{ language_data('Manage') }}</a>
                                                <a class="btn btn-danger btn-xs cdelete" id="{{ $airtime->id }}"
                                                    href="{{ url('sms/delete-airtime-bundle/' . $airtime->id) }}"> <i
        ...
        $(document).ready(function() {
            ...
            $("body").delegate(".cdelete", "click", function(e) {
                ...
                bootbox.confirm("{!! language_data('Are you sure') !!}?", function(result) {
                    if (result) {
                        var _url = $("#_url").val();
                        window.location.href = _url + "/sms/delete-airtime-bundle/" + id;
                    }
                });
            });
