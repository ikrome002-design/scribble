...
@section('content')
    <section class="wrapper-bottom-sec">
        <div class="p-30">
            <h2 class="page-title">{{ $staff->fname }} {{ $staff->lname }} ({{ $staff->email }})<br>
                Unique Id :{{ $staff->unique_id }}
            </h2>
            <p class="text-info fs-5">
                If payment is paybill and customer enters this unique id as account number, this staff will receive
                transaction
                confirmation sms. If customer enters their phone number as account number, they will instead receive thank
                you
                message.
                Othwerwise, staff assigned role to receive transaction sms will receive sms or the owner will receive
                confirmation sms.</p>
                            ...
                            <form action="" method="post">
                                <p class="text-info fs-5">
                                    *Lower roles will be automatically assigned if its super role
                                    ...
                                    effective.<br>
                                    *Please if you check re assign a role , ensure that the staff has a role to view staff
                                    in
                                    that
    ...
    <script>
        $('.checkbox-trans').on('change', function() {
            ...
            if ($(`.roles-check${sub_id}:checkbox:checked`).length == 0) {
                $(`#${roles}`).prop('checked', false).attr('readonly', true).attr('readonly', true)
            } else {
